...
<xsl:stylesheet 
    ...
    <xsl:template match="text()[ancestor::tei:seg][ancestor::j:streamText][not(ancestor::tei:w)][normalize-space(.)]">
		<xsl:analyze-string select="." regex="([\p{{L}}\p{{M}}\p{{N}}\p{{S}}\p{{C}}]+)|(\p{{P}})">
			<xsl:matching-substring>
				<xsl:variable name="word-chars" select="regex-group(1)" as="xs:string"/>
				<xsl:variable name="punct-chars" select="regex-group(2)" as="xs:string"/>
				<xsl:choose>
